...
- LLM fallback: **Gemini** (model cascade) → **local OpenAI-compatible** (Ollama/LM Studio) → **offline** template director + frame heuristic critic. Configure via `LLM_FALLBACK_ORDER`, `LOCAL_LLM_*`, `GEMINI_MODEL_FALLBACKS`.
  - Local setup: `ollama pull llama3.2` (director text) and `ollama pull llava` (critic vision). Set `LOCAL_LLM_MODEL` + `LOCAL_LLM_VISION_MODEL` — text-only models cannot review frames; vision route uses Ollama `/api/chat` with `format=json` + images.
